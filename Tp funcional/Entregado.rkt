...
#! Se define la lista ordenada de localidades
(define localidades
  '("Córdoba Capital" "Carlos Paz" "Bialet Massé" "Valle Hermoso"
    "La Falda" "Huerta Grande" "La Cumbre" "Capilla Del Monte"))

#! Se define la lista de costos entre pares de localidades consecutivas Córdoba Capital -> Carlos Paz = 1500, Carlos Paz -> Bialet Massé = 1500
(define costos
  '(1500 1500 1000 1200 1000 1200 1600))

#! Se definen los horarios de salida disponibles desde cada localidad 
(define horarios
  '(((07 00) (10 00) (12 00)) 
    ((07 30) (10 30) (12 30))
    ((07 45) (10 45) (12 45))
...
(define (buscar-indice localidad lista_localidades)
  (let buscar-indice-recursivo ((resto-lista lista_localidades) (indice 0)) #! Se define dentro un proceso recursivo
    (cond
      [(empty? resto-lista) #f]   ;Si se recorre toda la lista y no se encuentra la localidad, devuelve #f
      [(equal? (car resto-lista) localidad) indice]  ;Si la localidad actual es la que se esta buscando, devuelve el indice
      [else (buscar-indice-recursivo (cdr resto-lista) (+ indice 1))])))  ;Si no se encuentra todavía, se sigue con el resto de la lista y se suma 1 al índice
