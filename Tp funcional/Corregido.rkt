...
(define (horarios-disponibles lista-horas hora)
  (cond
    ...
    [(compararHora (car lista-horas) hora) ; Si compararHora da [True], entonces se ira alistando el primer elemento de la lista-horas y se sigue recorriendo el resto de la lista
      (cons (car lista-horas) (horarios-disponibles (cdr lista-horas) hora))]
    [else 
      (horarios-disponibles (cdr lista-horas) hora)])) ; Sino, se sigue iterando la lista de horas.  
...
(define (buscar-indice localidad lista_localidades indice)
    (cond
      [(empty? lista_localidades) #f] ;Si se recorre toda la lista y no se encuentra la localidad, devuelve #f
      [(equal? (car lista_localidades) localidad) indice] ;Si la localidad actual es la que se esta buscando, devuelve el indice
      [else (buscar-indice localidad (cdr lista_localidades) (+ indice 1))])) ;Si no se encuentra todavía, se sigue con el resto de la lista y se suma 1 al índice
